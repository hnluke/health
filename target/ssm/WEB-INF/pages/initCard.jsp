...
<body>
<form id="submitForm" name="submitForm" action="manage/initCard/2" method="post">
            ...
            <div class="ui_text_indent">
                <div id="box_border">
                    <div id="box_bottom">
                        ...
                        请输入卡片数量：
                        <input type="text" name="cardNumber" class="ui_input_txt02" value="${number}"/>
                        <input type="submit" value="卡片初始化" class="ui_input_btn01"/>
                        ...
                        <c:if test="${!empty(cardsList)}">
                            <c:forEach var="card"  items="${cardsList}">
                                <tr>
                                    ...
                                    <td>${card.cardId}</td>
                                    <td>${card.cardNo}</td>
                                    <td>${card.cardMoney}</td>

                                    <td><a href="javascript:if(confirm('确实要删除吗?'))
                                    location='manage/initCard/3?cardId=${card.cardId}'">删除</a></td>
                                </tr>
                            </c:forEach>
                        </c:if>
                    </table>
